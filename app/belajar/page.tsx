...
// dummy data
const modules = [
  { id: 'aljabar', title: 'Aljabar', level: 'Dasar (SMP)', icon: Calculator, progress: 2, total: 5, color: 'bg-blue-500', locked: false },
  { id: 'geometri', title: 'Geometri', level: 'Menengah (SMA)', icon: Shapes, progress: 0, total: 4, color: 'bg-emerald-500', locked: true },
  { id: 'kalkulus', title: 'Kalkulus', level: 'Lanjut (SMA)', icon: LineChart, progress: 0, total: 6, color: 'bg-purple-500', locked: true },
  { id: 'statistika', title: 'Statistika', level: 'Dasar (SD)', icon: Hash, progress: 5, total: 5, color: 'bg-orange-500', locked: false },
];
...
export default function BelajarPage() {
  ...
  const pageContent = (
    <div className="max-w-6xl mx-auto space-y-8 pt-4">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Halo {user.firstName || user.username || 'Pelajar'}, Lanjutkan Belajarmu!</h1>
        <p className="text-slate-500 dark:text-slate-400">Berikut adalah rekomendasi modul belajar berdasarkan hasil tes diagnostik terakhirmu.</p>
      </div>
          ...
          <div key={mod.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition">
            <div className={`h-24 ${mod.color} p-6 flex items-center justify-between relative overflow-hidden`}>
               <div className="relative z-10 text-white">
                 ...
                 <p className="text-sm opacity-90">{mod.level}</p>
               </div>
            ...
            <div className="p-5">
               <div className="flex justify-between text-sm font-medium text-slate-500 mb-2">
                 <span>Progress</span>
                 <span>{mod.progress}/{mod.total} Video</span>
               </div>
               ...
               >
                 {mod.locked ? (
                   <><Lock className="h-4 w-4" /> Terkunci (Selesaikan tes dulu)</>
                 ) : (
                   <><PlayCircle className="h-4 w-4" /> Mulai Belajar</>
                 )}
               </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
           <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl overflow-hidden flex flex-col shadow-2xl">
             <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
               <div>
                 <h3 className="font-bold text-xl text-slate-900 dark:text-white">Modul Pembelajaran</h3>
                 <p className="text-sm text-slate-500">Materi Video & Latihan Soal</p>
               </div>
             ...
             <div className="p-6 bg-slate-50 dark:bg-slate-950 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
               ...
               <div className="space-y-2 mt-4">
                 <h4 className="font-bold text-slate-900 dark:text-white">Daftar Putar (Playlist)</h4>
                 {[1, 2, 3].map(i => (
                   <div key={i} className="flex items-center gap-4 p-3 rounded-xl hover:bg-white dark:hover:bg-slate-900 border border-transparent hover:border-slate-200 dark:hover:border-slate-800 transition cursor-pointer">
                     ...
                     <div className="flex-1">
                       <p className="text-sm font-semibold text-slate-900 dark:text-white">Konsep Dasar Bagian {i}</p>
                       <p className="text-xs text-slate-500">Video • 05:{i}0 menit</p>
                     </div>
                   </div>
                 ))}
               </div>
